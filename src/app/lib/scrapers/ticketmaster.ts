...
export async function fetchAllTicketmasterEvents(): Promise<TicketmasterEvent[]> {
    ...
    while (hasMore && page < MAX_PAGES) {
        try {
            ...
            console.log(`   Page ${page + 1}: ${uniqueEventsMap.size} unique events`);
            page++;
...
export function normaliseTicketmasterEvent(
    ...
    // Use centralized category mapper
    ...
    return {
        ...
        dates: {
            ...existing.dates,
                ...
                localDate:
                    incoming.dates.end?.localDate || incoming.dates.start.localDate,
            },
        },
    };
}
...
function parseDate(date: string | undefined, time?: string): Date | undefined {
    ...
    try {
        ...
        console.warn(`⚠️  Error parsing date: ${date}, ${time}`);
        return undefined;
    }
}
